...
export const EditAlertDialog = ({ open, onOpenChange, onSuccess, alert }: EditAlertDialogProps) => {
  ...
  useEffect(() => {
    if (alert) {
      setFormData({
        ticker: alert.ticker || "",
        alert_type: alert.alert_type || "price_variation",
        threshold_value: alert.threshold_value?.toString() || "",
        target_price: (alert as any).target_price?.toString() || "",
        comparison_type: alert.comparison_type || "above",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.alert_type === "price_variation" || formData.alert_type === "price_drop") {
        updateData.threshold_value = formData.threshold_value ? parseFloat(formData.threshold_value) : null;
        updateData.target_price = null;
      } else if (formData.alert_type === "target_price") {
        updateData.target_price = formData.target_price ? parseFloat(formData.target_price) : null;
        updateData.threshold_value = null;
      } else if (formData.alert_type === "fixed_income_maturity") {
        updateData.threshold_value = formData.threshold_value ? parseFloat(formData.threshold_value) : 30;
        updateData.target_price = null;
      } else {
        updateData.threshold_value = null;
        updateData.target_price = null;
